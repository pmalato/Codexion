...
/*   Updated: 2026/07/25 11:42:13 by pmalato          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coder	*coder_list(t_arguments *parsed, t_dongle *d_list)
{
	...
	i = 0;
	len = parsed->number_of_coders;
	c_list = malloc(sizeof(t_coder) * len);
	if (!c_list)
		return (NULL);
	while (i < len)
	{
		...
		if (!thread)
			return (NULL);
		thread_fail = pthread_create(&c_list[i].thread, NULL, coder_routine, (void *)thread);
		if (thread_fail)
		{
			j = 0;
			while (j < i)
			{
				pthread_join(c_list[j].thread, NULL);
				j++;
			}
			return (NULL);
		}
